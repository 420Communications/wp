...
if ( ! function_exists( 'direo_comment' ) ) {
	function direo_comment( $comment, $args, $depth ) {
		 ...
		 $edit_icon  = class_exists( 'Directorist_Base' ) ? directorist_icon( 'la la-edit', false ) : '';
		 $reply_icon = class_exists( 'Directorist_Base' ) ? directorist_icon( 'la la-reply', false ) : '';
		 ?>
				...
				<div class="media-body">
					<div class="media_top">
						...
						<div class="heading_right">
							<?php edit_comment_link( $edit_icon . esc_html__( 'Edit', 'direo' ) ); ?>
							<?php
							comment_reply_link(
								array_merge(
									array( 'reply_text' => $reply_icon . esc_html__( ' Reply', 'direo' ) ),
									array(
										'depth'     => $depth,
										'max_depth' => 4,
									)
...
function direo_comment_form_config( $form ) {
	...
	$form['submit_button']        = '<input name="%1$s" type="submit" id="%2$s" class="btn btn-primary %3$s" value="' . esc_html__( 'Post Comment', 'direo' ) . '">';
