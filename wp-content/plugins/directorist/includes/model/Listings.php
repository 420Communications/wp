...
class Directorist_Listings {
	...
	public function openstreet_map_card_data() {
		...
		if ( ! empty( $listings->ids ) ) :
			...
			foreach ( $listings->ids as $listings_id ) :
				...
				$author_id = get_post_field ('post_author', $listings_id);
				$user_type = get_user_meta( $author_id, '_user_type', true );
				$has_subscription = $this->diero_has_subscription_for_larger_map($author_id);
				...
				// Code comment by by ASP - 12-09-2022
				...
				$map_data[] = [
					...
					// 'cat_icon'  => $this->loop_map_cat_icon(), Code comment by by ASP - 12-09-2022
					'cat_icon'  => $cat_icon,
				];
    ...
    public function diero_has_subscription_for_larger_map($author_id) {
		$subsProductArr = array();
		$currentUserType = get_user_meta( $author_id, '_user_type', true );
